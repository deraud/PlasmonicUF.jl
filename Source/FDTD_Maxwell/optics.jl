...
function calculateR!(opt::OpticsProperties)
    Threads.@threads for i in 1:Nx-1
        for j in 1:Ny-1
            opt.R[i,j] = ((opt.n[i,j]-1)^2+opt.k[i,j]^2)/((opt.n[i,j]+1)^2+opt.k[i,j]^2)
        end
    end
end

function calculateIndexN!(lb::Lattice, opt::OpticsProperties)
    Threads.@threads for i in trunc(Int, Nx/2):Nx
        for j in 1:Ny
            opt.n[i,j] = interpolate_value_n(lb.Tₑ[i,j])
        end
    end
